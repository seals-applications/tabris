...
async function saveDiagnosisHistory(diagnosisData) {
  ...
  try {
    ...
    // 現在のユーザー情報を取得
    const currentUser = window.currentSalesUser || JSON.parse(localStorage.getItem('current_sales_user') || '{}');
    console.log('saveDiagnosisHistory: currentUser =', currentUser);
    ...
    console.log('saveDiagnosisHistory: 保存データ =', insertData);
...
// 現場IDで診断履歴を取得
async function getDiagnosisHistoryBySiteId(siteId, limit = 50) {
  if (!supabase) {
    console.warn('Supabaseクライアントが初期化されていません');
    return [];
  }
  ...
  try {
    ...
    if (error) {
      console.error('現場IDでの診断履歴取得に失敗:', error);
      return [];
    }

    return data || [];
  } catch (error) {
    console.error('現場IDでの診断履歴取得中にエラーが発生:', error);
    return [];
  }
}
